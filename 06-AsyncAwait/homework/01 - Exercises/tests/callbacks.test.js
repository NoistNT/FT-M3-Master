...
const dirpathOne = path.join(__dirname, '../poem-one')
const dirpathTwo = path.join(__dirname, '../poem-two')
...
describe('01 | Ejercicios - Callbacks (poem-one)', () => {
  ...
  beforeEach(() => {
    jest.spyOn(utils, 'readFile').mockImplementation((filename, callback) => {
      const stanza = stanzasOne.find((stanza) => stanza.includes(filename))
...
describe('02 | Ejercicios - Callbacks (poem-two)', () => {
  ...
  beforeEach(() => {
    jest.spyOn(utils, 'readFile').mockImplementation((filename, callback) => {
      const stanza = stanzasTwo.find((stanza) => stanza.includes(filename))
      callback(null, stanza)
    })
  })
